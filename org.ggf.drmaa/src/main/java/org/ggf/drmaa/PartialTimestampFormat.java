 ...
 * Minor updates for using static constants of Calendar and PartialTimeStap.
 */
/*___INFO__MARK_END__*/
/*
...
/**
 * This class coverts a PartialTimestamp to and from the DRMAA specified time string. In order to represent a
 * PartialTimestamp object as a string, the PartialTimestamp object cannot have an unset field that is less significant
 * than the most significant set field. That is to say that if <code>CENTURY</code> is set, <code>YEAR</code>,
 * <code>MONTH</code>, and <code>DAY_OF_MONTH</code> must also be set. <code>SECONDS</code> and <code>ZONE_OFFSET</code>
 * are always optional.
 * 
 * <p>
 * Example:
...
public class PartialTimestampFormat extends Format {
  ...
  /**
   * Translates the PartialTimestamp into a DRMAA specified time string and appends the string to the given
   * StringBuffer. Since the PartialTimestampFormat class doesn't use fields, the fieldPosition parameter is ignored.
   * This method is equivalent to <code>stringBuffer.append (format (obj))</code>.
   * 
   * <p>
   * In order to represent a PartialTimestamp object as a string, the PartialTimestamp object cannot have an unset field
   * that is less significant than the most significant set field. TThat is to say that if <code>CENTURY</code> is set,
   * <code>YEAR</code>, <code>MONTH</code>, and <code>DAY_OF_MONTH</code> must also be set. <code>SECONDS</code> and
  ...
  public StringBuffer format(PartialTimestamp obj, StringBuffer stringBuffer, FieldPosition fieldPosition) {
    ...
    if (obj.isSet(PartialTimestamp.CENTURY)) {
      stringBuffer.append(nf.format(obj.get(PartialTimestamp.CENTURY)));
      fieldSet = true;
    }

    if (obj.isSet(Calendar.YEAR)) {
      stringBuffer.append(nf.format(obj.get(Calendar.YEAR)));
      fieldSet = true;
    } else if (fieldSet) {
      throw new IllegalArgumentException("In PartialTimestamp object, CENTURY is set but YEAR is not");
    }

    if (obj.isSet(Calendar.MONTH)) {
      if (fieldSet) {
        stringBuffer.append('/');
      }

      stringBuffer.append(nf.format(obj.get(Calendar.MONTH) + 1));
      fieldSet = true;
    } else if (fieldSet) {
      throw new IllegalArgumentException("In PartialTimestamp object, YEAR is set but MONTH is not");
    }

    if (obj.isSet(Calendar.DAY_OF_MONTH)) {
      if (fieldSet) {
        stringBuffer.append('/');
      }

      stringBuffer.append(nf.format(obj.get(Calendar.DAY_OF_MONTH)));
      fieldSet = true;
    } else if (fieldSet) {
      throw new IllegalArgumentException("In PartialTimestamp object, MONTH is set but DAY_OF_MONTH is not");
    }

    if (obj.isSet(Calendar.HOUR_OF_DAY)) {
      if (fieldSet) {
        stringBuffer.append(' ');
      }

      stringBuffer.append(nf.format(obj.get(Calendar.HOUR_OF_DAY)));
    } else {
      throw new IllegalArgumentException("In PartialTimestamp object, HOUR_OF_DAY is not set");
    }

    if (obj.isSet(Calendar.MINUTE)) {
      stringBuffer.append(':');
      stringBuffer.append(nf.format(obj.get(Calendar.MINUTE)));
    } else {
      throw new IllegalArgumentException("In PartialTimestamp object, MINUTE is not set");
    }

    if (obj.isSet(Calendar.SECOND)) {
      stringBuffer.append(':');
      stringBuffer.append(nf.format(obj.get(Calendar.SECOND)));
    }

    if (obj.isSet(Calendar.ZONE_OFFSET)) {
      int offset = obj.get(Calendar.ZONE_OFFSET);
  ...
  /**
   * Translates the PartialTimestamp into a DRMAA specified time string. This method is equivalent to
   * <code>format(obj, new StringBuffer (), new
  ...
  /**
   * Translates a DRMAA specified time string into a PartialTimestamp object. This method will parse as far as possible,
   * but after successfully parsing the <code>HOUR_OF_DAY</code> and <code>MINUTE</code> fields, if it encounters
   * unparsable text, it will stop and will <b>not</b> throw a java.text.ParseException.
   * 
  ...
  /**
   * Translates a DRMAA specified time string into a PartialTimestamp object. This method will parse as far as possible.
   * Upon completion, the parse position object will contain the index of the last character parsed.
   * 
   * @param str a DRMAA specified time string
  ...
  public Object parseObject(String str, ParsePosition parsePosition) {
    ...
    for (int index = parsePosition.getIndex(); index < chars.length; index++) {
      /*
       * If the character is a digit, and we're in a numeric field, and we haven't reached the end of the field, append
       * the character.
       */
      if ((field != 6) && (((field == 0) && (count < 4)) || ((field > 0) && (count < 2))) && Character.isDigit(chars[index])) {
        /*
         * If we're at the beginning of the minute, note where it started.
         */
        if ((field == 4) && (count == 0)) {
          minuteIndex = index;
        }
      ...
      /*
       * If the character is a colon, and we're not in the date, second, or offset sign fields, and we're at the end of
       * the field, advance to the next field.
       */
      else if ((field != 1) && (field != 2) && (field != 5) && (field != 6) && (count == 2) && (chars[index] == ':')) {
        /*
         * If we found the hour instead of the year, skip to the minute.
         */
        if (field == 0) {
          field = 4;
        } else {
      ...
      /*
       * If the character is a slash, and we're in the year or month, and we at the end of a field, advance to the next
       * field.
       */
      ...
      /*
       * If the character is a space, and we're not in the hour, offset sign, or offset hour, and we're at the end of
       * the field, advance to the next field.
       */
        ...
        if (!inWhitespace) {
          ...
          /*
           * If we found the offset instead of the second, skip to the offset.
           */
      ...
      /*
       * If the character is a minus or plus and we're in the offset field, set the offset field.
       */
      ...
      /*
       * If we're not in a patch of whitespace, we're in the minute, second, or offset minute, and we're at the end of
       * the field, and we can't parse the next character, we're done.
       */
      else if (!inWhitespace && (count == 2) && ((field == 4) || (field == 5) || (field == 8))) {
        parsePosition.setIndex(index);

        break;
      }
      /*
       * If we're in a patch of whitespace, and we're in the second or offset sign, and we can't parse the next
       * character, we're done, but we have to note the end of the string as being the beginning of the whitespace.
       */
      else if (inWhitespace && (count == 0) && ((field == 5) || (field == 6))) {
    ...
    if (Character.isDigit(fields[3][0])) {
      pt.set(Calendar.HOUR_OF_DAY, Integer.parseInt(new String(fields[3])));

      /* If the day field was set, parse the day, month, and year. */
      if (Character.isDigit(fields[2][0])) {
        pt.set(Calendar.DAY_OF_MONTH, Integer.parseInt(new String(fields[2])));
        pt.set(Calendar.MONTH, Integer.parseInt(new String(fields[1])) - 1);

        String year = new String(fields[0]).trim();

        /* If the year has four digits, parse the year and century. */
        if (year.length() == 4) {
          pt.set(Calendar.YEAR, Integer.parseInt(year.substring(2, 4)));
          pt.set(PartialTimestamp.CENTURY, Integer.parseInt(year.substring(0, 2)));
        }
        /* If the year has two digits, parse the year. */
        else {
          pt.set(Calendar.YEAR, Integer.parseInt(year));
        }
      }
      /*
       * If the day field wasn't set, but the month was, parse the day and month from the month and year fields.
       */
      else if (Character.isDigit(fields[1][0])) {
        pt.set(Calendar.DAY_OF_MONTH, Integer.parseInt(new String(fields[1])));
        ...
        if (month.length() != 2) {
          ...
          pt.set(Calendar.MONTH, Integer.parseInt(month) - 1);
        }
      }
      /*
       * If the day and month weren't set, but the year was, parse the day from the year field.
       */
        ...
        if (day.length() != 2) {
          ...
          pt.set(Calendar.DAY_OF_MONTH, Integer.parseInt(day));
        }
      }
    }
    /*
     * If the hour field wasn't set, but the year was, parse the hour from the year field.
     */
    else if (Character.isDigit(fields[0][0])) {
      ...
      if (hour.length() != 2) {
        ...
        pt.set(Calendar.HOUR_OF_DAY, Integer.parseInt(hour));
      }
    }
    ...
    if (Character.isDigit(fields[4][0])) {
      pt.set(Calendar.MINUTE, Integer.parseInt(new String(fields[4])));
    }
    ...
    if (Character.isDigit(fields[5][0])) {
      pt.set(Calendar.SECOND, Integer.parseInt(new String(fields[5])));
    }
    ...
    if ((fields[6][0] != ' ') && Character.isDigit(fields[7][0]) && Character.isDigit(fields[8][0])) {
      ...
      pt.set(Calendar.ZONE_OFFSET, offset);
    }
    /* If the offset is partially set, we have a problem. */
      ...
      if (fields[6][0] != ' ') {
        if (Character.isDigit(fields[7][0])) {
          if ((chars.length > tzIndex + 3) && (chars[tzIndex + 3] == ':')) {
            tzIndex += 4;
          } else {
            tzIndex += 3;
          }
