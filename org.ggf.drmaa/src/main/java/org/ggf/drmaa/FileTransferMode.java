...
/**
 * This class represents the streams which should be used for file transfers. For each of the three properties which is
 * set to true, the corresponding stream's path property in the job template will be treated as a source or destination
 * (depending on the stream) for file tranfers. For example, if the inputStream property is set to true, the inputPath
 * property of the JobTemplate will be interpreted as a source from which to transfer files.
 * 
...
public class FileTransferMode implements Serializable, Cloneable {
   ...
   * @return whether the FileTransferMode object has the same property settings as this one
   */
